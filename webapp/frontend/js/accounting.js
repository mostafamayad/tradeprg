...
function openArPaymentModal() {
    ...
    document.getElementById('modal-body').innerHTML = `
        <div id="ar-bal-preview" style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px">
            <div style="background:linear-gradient(135deg,#fff1f2 0%,#ffe4e6 100%);border:1.5px solid #fecdd3;border-radius:12px;padding:14px 12px;text-align:center">
                <div style="width:36px;height:36px;background:#ef4444;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 8px">
                    <i class="fa-solid fa-file-invoice-dollar" style="color:#fff;font-size:15px"></i>
                </div>
                <div style="font-size:0.7rem;color:#9f1239;font-weight:600;margin-bottom:4px;letter-spacing:0.3px">إجمالي المديونية</div>
                <strong id="ar-pay-bal-current" style="font-size:1.05rem;display:block;line-height:1.3">---</strong>
            </div>
            <div style="background:linear-gradient(135deg,#f5f3ff 0%,#ede9fe 100%);border:1.5px solid #ddd6fe;border-radius:12px;padding:14px 12px;text-align:center">
                <div style="width:36px;height:36px;background:#7c3aed;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 8px">
                    <i class="fa-solid fa-coins" style="color:#fff;font-size:15px"></i>
                </div>
                <div style="font-size:0.7rem;color:#5b21b6;font-weight:600;margin-bottom:4px;letter-spacing:0.3px">المبلغ المُسدَّد</div>
                <strong id="ar-pay-bal-amount" style="font-size:1.05rem;color:#7c3aed;display:block;line-height:1.3">---</strong>
            </div>
            <div style="background:linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%);border:1.5px solid #bbf7d0;border-radius:12px;padding:14px 12px;text-align:center">
                <div style="width:36px;height:36px;background:#10b981;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 8px">
                    <i class="fa-solid fa-scale-balanced" style="color:#fff;font-size:15px"></i>
                </div>
                <div style="font-size:0.7rem;color:#065f46;font-weight:600;margin-bottom:4px;letter-spacing:0.3px">الرصيد بعد السداد</div>
                <strong id="ar-pay-bal-after" style="font-size:1.05rem;display:block;line-height:1.3">---</strong>
            </div>
        </div>
        <div class="form-grid">
    ...
    // Helper to format balance with color label
    function arFmtBal(val) {
        const fmt = window.formatMoney || (v => Number(v).toLocaleString('ar-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        if (val > 0.009) return `<span style="color:#ef4444;font-weight:bold">عليه ${fmt(val)}</span>`;
        if (val < -0.009) return `<span style="color:#10b981;font-weight:bold">له ${fmt(Math.abs(val))}</span>`;
        return `<span style="color:#64748b">صِفر</span>`;
    }

    let arCurrentBalance = 0;

    function arUpdateAfter() {
        const amt = parseFloat(document.getElementById('modal-ar-pay-amount')?.value) || 0;
        const fmt = window.formatMoney || (v => Number(v).toLocaleString('ar-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        document.getElementById('ar-pay-bal-amount').innerHTML = amt > 0 ? fmt(amt) : '---';
        document.getElementById('ar-pay-bal-after').innerHTML = arFmtBal(arCurrentBalance - amt);
    }
    ...
    document.getElementById('modal-ar-pay-cust').addEventListener('change', async function() {
        const cid = this.value;
        document.getElementById('ar-pay-bal-current').innerHTML = cid ? '<i class="fa-solid fa-spinner fa-spin" style="color:#94a3b8"></i>' : '---';
        document.getElementById('ar-pay-bal-after').innerHTML = '---';
        document.getElementById('ar-pay-bal-amount').innerHTML = '---';
        arCurrentBalance = 0;
        if (!cid) return;
        try {
            const r = await window.req('/collections/customer/' + cid + '/preview');
            if (r && r.data) {
                arCurrentBalance = r.data.balance || 0;
                document.getElementById('ar-pay-bal-current').innerHTML = arFmtBal(arCurrentBalance);
                arUpdateAfter();
            }
        } catch(e) { console.error(e); }
    });

    document.getElementById('modal-ar-pay-amount').addEventListener('input', arUpdateAfter);
...
window.initArMatching = async function() {
    ...
    let paymentsData = [];
    let invoicesData = [];
    let selectedPayment = null; // The one payment being distributed

    // Plain number formatter (no HTML spans) — safe for textContent/dropdown
    function fm(v) {
        const n = Number(v || 0);
    ...
    try {
        ...
        if (res && res.data) {
            res.data.forEach(c => {
                const opt = document.createElement('option');
                opt.value = c.id;
                opt.textContent = c.customer_name + (c.current_balance ? ' — رصيد: ' + fm(c.current_balance) : '');
                customerSel.appendChild(opt);
            });
        }
    } catch (e) { console.error('Load matching customers error:', e); }

    // Update summary bar (banner + footer totals)
    function updateSummary() {
        if (!selectedPayment) return;
        let totalAmt = 0, totalDisc = 0;
        document.querySelectorAll('#ar-matching-invoices-tbody tr.inv-row').forEach(tr => {
            const amt = num(tr.querySelector('.inline-alloc-amt')?.value);
            const disc = num(tr.querySelector('.inline-alloc-disc')?.value);
            const remaining = num(tr.dataset.invRemaining);
            const afterAlloc = Math.max(0, remaining - amt - disc);
            const afterTd = tr.querySelector('.after-alloc-td');
            if (afterTd) {
                afterTd.textContent = fm(afterAlloc);
                afterTd.style.color = (amt > 0 || disc > 0) ? '#10b981' : '';
                afterTd.style.fontWeight = (amt > 0 || disc > 0) ? '600' : '';
            }
            // Highlight row if any amount entered
            if (amt > 0 || disc > 0) {
        ...
        const payAvail = num(selectedPayment.unallocated);
        const remaining = payAvail - totalAmt;

        // Update banner
        document.getElementById('ar-banner-distributed').textContent = fm(totalAmt);
        document.getElementById('ar-banner-remaining').textContent = fm(Math.max(0, remaining));
        document.getElementById('ar-banner-remaining').style.color = remaining < 0 ? '#fca5a5' : '#fff';

        // Update footer
        document.getElementById('ar-matching-total-alloc').textContent = fm(totalAmt) + ' ج.م';
        document.getElementById('ar-matching-total-disc').textContent = fm(totalDisc) + ' ج.م';
        document.getElementById('ar-matching-remaining-payments').textContent = fm(Math.max(0, remaining)) + ' ج.م';

        // Enable save if anything was entered and no overdraft on payment
        const hasData = totalAmt > 0;
        const overdraft = totalAmt > payAvail + 0.001;
        const saveBtn = document.getElementById('btn-ar-matching-save');
        if (saveBtn) {
            saveBtn.disabled = !hasData || overdraft;
            saveBtn.style.background = overdraft ? '#ef4444' : '';
        }
    }

    // Select a payment voucher and render invoices with inline inputs
    function selectPayment(p) {
        selectedPayment = p;

        // Highlight selected row
        document.querySelectorAll('#ar-matching-payments-tbody tr.pay-row').forEach(tr => {
            tr.style.background = String(tr.dataset.payId) === String(p.id) ? '#ede9fe' : '';
            const radio = tr.querySelector('.pay-radio');
            if (radio) radio.checked = String(tr.dataset.payId) === String(p.id);
        });

        // Show banner
        const banner = document.getElementById('ar-selected-payment-banner');
        if (banner) {
            banner.style.display = 'block';
            const methodMap = { cash: 'نقدي', cheque: 'شيك', transfer: 'تحويل', other: 'أخرى' };
            const methodLabel = methodMap[p.payment_method] || p.payment_method || '';
            document.getElementById('ar-banner-pay-no').textContent = p.payment_no + (methodLabel ? ' — ' + methodLabel : '');
            document.getElementById('ar-banner-total').textContent = fm(p.amount);
            document.getElementById('ar-banner-distributed').textContent = '0.00';
            document.getElementById('ar-banner-remaining').textContent = fm(p.unallocated);
            document.getElementById('ar-banner-remaining').style.color = '#fff';
        }

        // Update hint
        const hint = document.getElementById('ar-inv-hint');
        if (hint) hint.textContent = 'أدخل المبلغ أمام الفاتورة المراد إغلاقها ← السند: ' + p.payment_no;

        // Render invoices with inline inputs
        const invTbody = document.getElementById('ar-matching-invoices-tbody');
        if (!invTbody) return;

        if (invoicesData.length === 0) {
            invTbody.innerHTML = '<tr><td colspan="7" class="text-center" style="padding:30px;color:var(--text-muted)">لا توجد فواتير غير مسددة لهذا العميل</td></tr>';
            return;
        }
        ...
        invoicesData.forEach(inv => {
            ...
            tr.dataset.invRemaining = inv.remaining;
            tr.style.transition = 'background .2s';
            tr.innerHTML = `
                <td><strong>${inv.invoice_no}</strong></td>
                <td>${inv.invoice_date ? inv.invoice_date.substring(0,10) : ''}</td>
                <td>${fm(inv.grand_total)}</td>
                <td style="color:#ef4444;font-weight:600">${fm(inv.remaining)}</td>
                <td>
                    <input type="number" class="inline-alloc-amt" min="0" max="${inv.remaining}" step="0.01" placeholder="0.00"
                        style="width:130px;padding:6px 8px;border:2px solid #e2e8f0;border-radius:8px;text-align:center;font-size:14px;font-weight:600;outline:none;transition:.2s;color:var(--primary-color)"
                        onfocus="this.style.borderColor='var(--primary-color)'" onblur="this.style.borderColor='#e2e8f0'">
                </td>
                <td>
                    <input type="number" class="inline-alloc-disc" min="0" step="0.01" placeholder="0.00"
                        style="width:120px;padding:6px 8px;border:2px solid #e2e8f0;border-radius:8px;text-align:center;font-size:14px;font-weight:600;outline:none;transition:.2s;color:#f59e0b"
                        onfocus="this.style.borderColor='#f59e0b'" onblur="this.style.borderColor='#e2e8f0'">
                </td>
                <td class="after-alloc-td" style="font-weight:600">${fm(inv.remaining)}</td>
            `;
            tr.querySelector('.inline-alloc-amt').addEventListener('input', updateSummary);
            tr.querySelector('.inline-alloc-disc').addEventListener('input', updateSummary);
    ...
    function clearSelection() {
        ...
        const hint = document.getElementById('ar-inv-hint');
        if (hint) hint.textContent = '← اختر سند القبض من الجدول أعلاه أولاً';
        // Reset invoice table
        const invTbody = document.getElementById('ar-matching-invoices-tbody');
        if (invTbody) invTbody.innerHTML = '<tr><td colspan="7" class="text-center" style="padding:30px;color:var(--text-muted)">اختر سند القبض من الجدول أعلاه</td></tr>';
        // Reset totals
        ['ar-matching-total-alloc','ar-matching-total-disc','ar-matching-remaining-payments'].forEach(id => {
            const el = document.getElementById(id);
    ...
    document.getElementById('ar-banner-clear')?.addEventListener('click', clearSelection);

    // Load customer data
    async function loadMatchingData() {
        const cid = customerSel.value;
        if (!cid) {
            window.API.showAlert ? window.API.showAlert('اختر العميل أولاً', { type: 'warning' }) : alert('اختر العميل أولاً');
            return;
        }
        clearSelection();
        try {
            ...
            // Render payment vouchers
            const payTbody = document.getElementById('ar-matching-payments-tbody');
            if (payTbody) {
                payTbody.innerHTML = '';
                if (paymentsData.length === 0) {
                    payTbody.innerHTML = '<tr><td colspan="6" class="text-center" style="padding:30px;color:var(--text-muted)"><i class="fa-solid fa-circle-check" style="color:#10b981;font-size:20px;margin-left:8px"></i>لا توجد سندات قبض بحاجة إلى توزيع</td></tr>';
                } else {
                    paymentsData.forEach(p => {
                        ...
                        const methodLabel = methodMap[p.payment_method] || p.payment_method || '—';
                        tr.innerHTML = `
                            <td><input type="radio" name="ar-pay-radio" class="pay-radio" style="width:16px;height:16px;accent-color:var(--primary-color);cursor:pointer"></td>
                            <td><strong style="color:var(--primary-color)">${p.payment_no}</strong></td>
                            <td>${p.payment_date ? p.payment_date.substring(0,10) : ''}</td>
                            <td><span style="background:#ede9fe;color:#6d28d9;padding:3px 10px;border-radius:20px;font-size:12px">${methodLabel}</span></td>
                            <td>${fm(p.amount)}</td>
                            <td><strong style="color:#10b981;font-size:15px">${fm(p.unallocated)}</strong></td>
                        `;
                        tr.addEventListener('click', () => selectPayment(p));
                        payTbody.appendChild(tr);
                    });
                }
            }

            // If only one payment, auto-select it
            if (paymentsData.length === 1) selectPayment(paymentsData[0]);
    ...
    document.getElementById('btn-ar-matching-save')?.addEventListener('click', async () => {
        const cid = customerSel.value;
        if (!cid || !selectedPayment) return;

        const allocations = [];
        document.querySelectorAll('#ar-matching-invoices-tbody tr.inv-row').forEach(tr => {
            const iid = parseInt(tr.dataset.invId);
            const amt = num(tr.querySelector('.inline-alloc-amt')?.value);
            const disc = num(tr.querySelector('.inline-alloc-disc')?.value);
            if (amt > 0 || disc > 0) {
                allocations.push({
        ...
        if (allocations.length === 0) {
            window.API.showAlert ? window.API.showAlert('أدخل مبلغاً أمام فاتورة واحدة على الأقل', { type: 'warning' }) : alert('أدخل مبلغاً أمام فاتورة واحدة على الأقل');
            return;
        }

        const totalAlloc = allocations.reduce((s, a) => s + a.allocated_amount, 0);
        if (totalAlloc > num(selectedPayment.unallocated) + 0.001) {
            window.API.showAlert ? window.API.showAlert('إجمالي المبالغ الموزعة يتجاوز قيمة السند المتاحة', { type: 'error' }) : alert('إجمالي المبالغ الموزعة يتجاوز قيمة السند المتاحة');
            return;
        }
...
function openApPaymentModal() {
    ...
    document.getElementById('modal-body').innerHTML = `
        <div id="ap-bal-preview" style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px">
            <div style="background:linear-gradient(135deg,#fff7ed 0%,#fed7aa 100%);border:1.5px solid #fdba74;border-radius:12px;padding:14px 12px;text-align:center">
                <div style="width:36px;height:36px;background:#f97316;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 8px">
                    <i class="fa-solid fa-truck" style="color:#fff;font-size:15px"></i>
                </div>
                <div style="font-size:0.7rem;color:#9a3412;font-weight:600;margin-bottom:4px;letter-spacing:0.3px">إجمالي المستحق للمورد</div>
                <strong id="ap-pay-bal-current" style="font-size:1.05rem;display:block;line-height:1.3">---</strong>
            </div>
            <div style="background:linear-gradient(135deg,#f5f3ff 0%,#ede9fe 100%);border:1.5px solid #ddd6fe;border-radius:12px;padding:14px 12px;text-align:center">
                <div style="width:36px;height:36px;background:#7c3aed;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 8px">
                    <i class="fa-solid fa-money-bill-wave" style="color:#fff;font-size:15px"></i>
                </div>
                <div style="font-size:0.7rem;color:#5b21b6;font-weight:600;margin-bottom:4px;letter-spacing:0.3px">المبلغ المُسدَّد</div>
                <strong id="ap-pay-bal-amount" style="font-size:1.05rem;color:#7c3aed;display:block;line-height:1.3">---</strong>
            </div>
            <div style="background:linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%);border:1.5px solid #bbf7d0;border-radius:12px;padding:14px 12px;text-align:center">
                <div style="width:36px;height:36px;background:#10b981;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 8px">
                    <i class="fa-solid fa-scale-balanced" style="color:#fff;font-size:15px"></i>
                ...
                <div style="font-size:0.7rem;color:#065f46;font-weight:600;margin-bottom:4px;letter-spacing:0.3px">الرصيد بعد السداد</div>
                <strong id="ap-pay-bal-after" style="font-size:1.05rem;display:block;line-height:1.3">---</strong>
            </div>
        </div>
        <div class="form-grid">
    ...
    // Helpers
    function apFmtBal(val) {
        const fmt = window.formatMoney || (v => Number(v).toLocaleString('ar-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        if (val > 0.009) return `<span style="color:#f97316;font-weight:bold">له ${fmt(val)}</span>`;
        if (val < -0.009) return `<span style="color:#10b981;font-weight:bold">عليه ${fmt(Math.abs(val))}</span>`;
        return `<span style="color:#64748b">صِفر</span>`;
    }

    let apCurrentBalance = 0;

    function apUpdateAfter() {
        const amt = parseFloat(document.getElementById('modal-ap-pay-amount')?.value) || 0;
        const fmt = window.formatMoney || (v => Number(v).toLocaleString('ar-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        document.getElementById('ap-pay-bal-amount').innerHTML = amt > 0 ? fmt(amt) : '---';
        document.getElementById('ap-pay-bal-after').innerHTML = apFmtBal(apCurrentBalance - amt);
    }
    ...
    document.getElementById('modal-ap-pay-supplier').addEventListener('change', async function() {
        const sid = this.value;
        document.getElementById('ap-pay-bal-current').innerHTML = sid ? '<i class="fa-solid fa-spinner fa-spin" style="color:#94a3b8"></i>' : '---';
        document.getElementById('ap-pay-bal-after').innerHTML = '---';
        document.getElementById('ap-pay-bal-amount').innerHTML = '---';
        apCurrentBalance = 0;
        if (!sid) return;
        try {
            const r = await window.req('/payments/supplier/' + sid + '/preview');
            if (r && r.data) {
                apCurrentBalance = r.data.balance || 0;
                document.getElementById('ap-pay-bal-current').innerHTML = apFmtBal(apCurrentBalance);
                apUpdateAfter();
            }
        } catch(e) { console.error(e); }
    });

    document.getElementById('modal-ap-pay-amount').addEventListener('input', apUpdateAfter);
...
window.initApMatching = async function() {
    ...
    try {
        ...
        if (res && res.data) {
            res.data.forEach(s => {
                const opt = document.createElement('option');
                opt.value = s.id;
                opt.textContent = s.supplier_name + (s.current_balance ? ' — رصيد: ' + fm(s.current_balance) : '');
    ...
    function updateSummary() {
        ...
        document.querySelectorAll('#ap-matching-invoices-tbody tr.inv-row').forEach(tr => {
            ...
            tr.style.background = (amt > 0 || disc > 0) ? '#f0fdf4' : '';
            totalAmt += amt;
            totalDisc += disc;
        });
        ...
        document.getElementById('ap-banner-distributed').textContent = fm(totalAmt);
        document.getElementById('ap-banner-remaining').textContent = fm(Math.max(0, remaining));
        document.getElementById('ap-banner-remaining').style.color = remaining < 0 ? '#fca5a5' : '#fff';

        document.getElementById('ap-matching-total-alloc').textContent = fm(totalAmt) + ' ج.م';
        document.getElementById('ap-matching-total-disc').textContent = fm(totalDisc) + ' ج.م';
        document.getElementById('ap-matching-remaining-payments').textContent = fm(Math.max(0, remaining)) + ' ج.م';
    ...
    function selectPayment(p) {
        ...
        document.querySelectorAll('#ap-matching-payments-tbody tr.pay-row').forEach(tr => {
            tr.style.background = String(tr.dataset.payId) === String(p.id) ? '#e0f2fe' : '';
            const radio = tr.querySelector('.pay-radio');
            if (radio) radio.checked = String(tr.dataset.payId) === String(p.id);
        });

        const banner = document.getElementById('ap-selected-payment-banner');
        if (banner) {
            banner.style.display = 'block';
            const methodMap = { cash: 'نقدي', cheque: 'شيك', transfer: 'تحويل', other: 'أخرى' };
            const methodLabel = methodMap[p.payment_method] || p.payment_method || '';
            document.getElementById('ap-banner-pay-no').textContent = p.payment_no + (methodLabel ? ' — ' + methodLabel : '');
            document.getElementById('ap-banner-total').textContent = fm(p.amount);
            document.getElementById('ap-banner-distributed').textContent = '0.00';
            document.getElementById('ap-banner-remaining').textContent = fm(p.unallocated);
            document.getElementById('ap-banner-remaining').style.color = '#fff';
        }

        const hint = document.getElementById('ap-inv-hint');
        if (hint) hint.textContent = 'أدخل المبلغ أمام الفاتورة المراد إغلاقها ← السند: ' + p.payment_no;
        ...
        if (invoicesData.length === 0) {
            invTbody.innerHTML = '<tr><td colspan="7" class="text-center" style="padding:30px;color:var(--text-muted)">لا توجد فواتير غير مسددة لهذا المورد</td></tr>';
            return;
        }
    ...
    function clearSelection() {
        ...
        if (banner) banner.style.display = 'none';
        const hint = document.getElementById('ap-inv-hint');
        if (hint) hint.textContent = '← اختر سند الصرف من الجدول أعلاه أولاً';
        const invTbody = document.getElementById('ap-matching-invoices-tbody');
        if (invTbody) invTbody.innerHTML = '<tr><td colspan="7" class="text-center" style="padding:30px;color:var(--text-muted)">اختر سند الصرف من الجدول أعلاه</td></tr>';
        ['ap-matching-total-alloc','ap-matching-total-disc','ap-matching-remaining-payments'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.textContent = '0.00 ج.م';
        });
        const saveBtn = document.getElementById('btn-ap-matching-save');
    ...
    document.getElementById('ap-banner-clear')?.addEventListener('click', clearSelection);

    async function loadMatchingData() {
        const sid = supplierSel.value;
        if (!sid) {
            window.API.showAlert ? window.API.showAlert('اختر المورد أولاً', { type: 'warning' }) : alert('اختر المورد أولاً');
            return;
        }
        clearSelection();
        try {
            ...
            if (payTbody) {
                payTbody.innerHTML = '';
                if (paymentsData.length === 0) {
                    payTbody.innerHTML = '<tr><td colspan="6" class="text-center" style="padding:30px;color:var(--text-muted)"><i class="fa-solid fa-circle-check" style="color:#10b981;font-size:20px;margin-left:8px"></i>لا توجد سندات صرف بحاجة إلى توزيع</td></tr>';
                } else {
                    paymentsData.forEach(p => {
                        const tr = document.createElement('tr');
                        tr.className = 'pay-row';
                        ...
                        const methodLabel = methodMap[p.payment_method] || p.payment_method || '—';
                        tr.innerHTML = `
                            <td><input type="radio" name="ap-pay-radio" class="pay-radio" style="width:16px;height:16px;accent-color:var(--primary-color);cursor:pointer"></td>
                            <td><strong style="color:var(--primary-color)">${p.payment_no}</strong></td>
                            <td>${p.payment_date ? p.payment_date.substring(0,10) : ''}</td>
                            <td><span style="background:#e0f2fe;color:#0369a1;padding:3px 10px;border-radius:20px;font-size:12px">${methodLabel}</span></td>
                            <td>${fm(p.amount)}</td>
                            <td><strong style="color:#10b981;font-size:15px">${fm(p.unallocated)}</strong></td>
                        `;
                        tr.addEventListener('click', () => selectPayment(p));
    ...
    document.getElementById('btn-ap-matching-save')?.addEventListener('click', async () => {
        ...
        try {
            ...
            console.error('Save AP matching error:', e);
            window.API.showAlert ? window.API.showAlert('خطأ في حفظ المطابقة: ' + (e.message || ''), { type: 'error' }) : alert('خطأ في حفظ المطابقة');
            saveBtn.innerHTML = origHtml;
            saveBtn.disabled = false;
        }
