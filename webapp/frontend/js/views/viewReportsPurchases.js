(function() {
    ...
    viewHandlers['view-reports-purchases'] = async function () {
        ...
        function getFilters() {
            ...
            const matchStatus = document.getElementById('pur-rpt-match-status')?.value || '';
            return { from, to, sup, store, matchStatus };
        }

        function updateFilterVisibility(reportType) {
            hide('pur-rpt-filter-supplier'); hide('pur-rpt-filter-store'); hide('pur-rpt-filter-match-status');
            ...
            if (reportType === 'pur-matching-status') { show('pur-rpt-filter-supplier'); show('pur-rpt-filter-match-status'); }
        }
        ...
        const renderers = {
            ...
            async 'pur-matching-status'() {
                const f = getFilters();
                const p = new URLSearchParams();
                if (f.from) p.append('from', f.from); if (f.to) p.append('to', f.to);
                if (f.sup) p.append('supplier_id', f.sup);
                if (f.matchStatus) p.append('status', f.matchStatus);
                const r = await req(`/reports/ap-matching-status?${p.toString()}`);
                if (!r.success || !r.data) { showError(r.message); return; }
                const data = r.data || [];
                const headers = ['الفاتورة', 'التاريخ', 'المورد', 'الإجمالي', 'المسدد', 'الخصم', 'المتبقي', 'الحالة'];
                const body = data.length === 0
                ...
                renderTable(headers, body, foot);
                _lastReportData = {
                    title: 'حالة مطابقة فواتير الموردين',
                    filters: [{ label: 'المورد', value: f.sup }, { label: 'الحالة', value: f.matchStatus }].filter(x => x.value),
                    summary: [{ label: 'عدد', value: data.length }],
                    columns: headers, rows: data.map(i => [i.invoice_no, i.invoice_date, i.supplier_name, fmt(i.grand_total), fmt(i.allocated_amount), fmt(i.discount_amount), fmt(i.remaining), i.match_status]),
                    totals: []
                };
            }
        };
